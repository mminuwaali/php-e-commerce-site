...
@section('content')
    <form action="{{ route('signin') }}" method="post" class="w-full md:w-3/4 bg-gray-50 p-8 self-center flex flex-col gap-8">
        @csrf
        @if (session('status'))
        ...
        <button
            class="px-8 py-2 uppecase font-bold border-2 w-56 self-center uppercase rounded-full text-gray-400 hover:bg-gray-400 hover:text-white">signin</button>
        <a href="{{ route('signup') }}" class="text-gray-600 font-thinbold">Don't have an account? <span
